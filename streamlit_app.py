...
# ── ML Pipeline ───────────────────────────────────────────────────────────────
@st.cache_data(show_spinner=False)
def _train_models_cached(mtime: str):
    ...
    eng = spine.sort_values(["zone_id", "time"]).copy() if "zone_id" in spine.columns else spine.copy()
    if "zone_id" in eng.columns:
        for lag in [1, 2, 24]:
            eng[f"occ_lag_{lag}h"] = eng.groupby("zone_id")["occupancy_rate"].shift(lag)
        eng["occ_roll_7d"] = (
            eng.groupby("zone_id")["occupancy_rate"]
    ...
    if "temp_c" in eng.columns and "acn_arrival_share" in eng.columns:
        eng["temp_x_arrival"] = eng["temp_c"] * eng["acn_arrival_share"]
    if "e_price_norm" in eng.columns:
        eng["price_bin"] = pd.cut(eng["e_price_norm"], bins=3, labels=[0, 1, 2]).astype(float)

    LAG_F = [c for c in ["occ_lag_1h", "occ_lag_2h", "occ_lag_24h", "occ_roll_7d",
                          "hour_sin", "hour_cos", "dow_sin", "dow_cos",
                          "temp_x_arrival", "price_bin"] if c in eng.columns]
    poi_cols = [c for c in eng.columns if c.startswith("poi_")]
    ALL_F = [f for f in (TIME_F + WEATHER_F + PRICE_F + ACN_F + INFRA_F + LAG_F + poi_cols)
             if f in eng.columns]

    if "time" in eng.columns:
        eng_s = eng.sort_values("time").reset_index(drop=True)
    else:
        eng_s = eng.reset_index(drop=True)

    n = len(eng_s)
    n_tr, n_v = int(n * 0.70), int(n * 0.85)
    all_v = ALL_F + [TARGET]
    tr = eng_s.iloc[:n_tr].dropna(subset=all_v)
    te = eng_s.iloc[n_v:].dropna(subset=all_v)
    X_tr, y_tr = tr[ALL_F].values, tr[TARGET].values
    X_te, y_te = te[ALL_F].values, te[TARGET].values

    # Baseline
    bl_cols = [f for f in TIME_F if f in eng.columns]
    lr = LinearRegression().fit(tr[bl_cols].values, y_tr)
    baseline_rmse = float(np.sqrt(mean_squared_error(y_te, lr.predict(te[bl_cols].values))))
    ...
    xgb_rmse = float(np.sqrt(mean_squared_error(y_te, xgb_pred)))
    xgb_mae  = float(mean_absolute_error(y_te, xgb_pred))
    ...
    metrics = {
        ...
        "xgb_rmse": xgb_rmse,
        "xgb_mae": xgb_mae,
        "improvement_pct": float((baseline_rmse - xgb_rmse) / baseline_rmse * 100),
        "n_features": len(ALL_F),
        "n_train": len(tr),
        "n_test": len(te),
    }
    feat_imp = pd.Series(xgb.feature_importances_, index=ALL_F).sort_values(ascending=False)
    medians = {c: float(spine[c].median()) for c in spine.columns if pd.api.types.is_numeric_dtype(spine[c])}

    return rf, xgb, ALL_F, metrics, feat_imp, y_te, xgb_pred, medians


def train_models():
    fused_path = OUTPUTS / "fused_all.csv"
    mtime = str(fused_path.stat().st_mtime) if fused_path.exists() else "0"
    return _train_models_cached(mtime)
...
def page_home():
    ...
    for col, (icon, q, method, color) in zip(q_cols, [
        ("🔮", "When will it be full?",   "Demand prediction + 48h forecasting",         "#3b82f6"),
        ("📍", "Where to build next?",    "Zone demand clustering + archetype analysis",  "#10b981"),
        ("🔌", "What charger type?",      "User behavior segmentation from real sessions","#f59e0b"),
    ]):
        with col:
            st.markdown(
    ...
    with col1:
        st.subheader("Three Dimensions of EV Charging Intelligence")
        try:
            n_acn = f"{len(acn):,}"
        except Exception:
            n_acn = "~28K"
        st.dataframe(pd.DataFrame([
            {"Dataset": "UrbanEV (Shenzhen, China)",  "Dimension": "Demand",   "Records": "4,344 hrs × 275 zones", "Key Info": "Occupancy, weather, POI, pricing"},
            {"Dataset": "ACN-Data (Caltech, USA)",    "Dimension": "Behavior", "Records": f"{n_acn} sessions",     "Key Info": "Arrival time, energy, dwell"},
            {"Dataset": "Kaggle Global Stations",     "Dimension": "Supply",   "Records": "5,000 stations",        "Key Info": "Charger type, capacity, ratings"},
        ]), use_container_width=True, hide_index=True)

    with col2:
        st.subheader("Analysis Modules")
        for w, m in [
            ("Week 1–2", "ML Workflow + Feature Engineering"),
            ("Week 3",   "Trees & Ensembles — RF + XGBoost"),
            ("Week 4",   "Clustering — zones + user behavior"),
            ("Week 5",   "NLP — Review Sentiment Analysis"),
            ("Week 6",   "Time Series — ARIMA + ARIMAX"),
            ("Week 7",   "GenAI Advisory — prompt builder"),
        ]:
            st.markdown(f"**✅ {w}** — {m}")

    st.divider()
    insight("🌏 <strong>India Angle:</strong> India targets <strong>30% EV penetration by 2030</strong> under FAME-III subsidies. This model transfers to Indian cities by swapping infrastructure profile features — no full data collection needed.")
    st.caption("Use the sidebar to navigate to each analytical section.")


# ─────────────────────────────────────────────────────────────────────────────
def page_data_explorer():
    sh("📊 Data Explorer", "Explore all three data sources with interactive visualizations")
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["🗂️ Dataset Overview", "🔥 Demand Heatmap", "🚗 ACN Sessions", "⚡ Station Profile"])
...
def page_demand_prediction():
    sh("🤖 Demand Prediction", "XGBoost & Random Forest trained on ~37 fused features")

    if not (OUTPUTS / "fused_all.csv").exists():
        st.warning("**outputs/fused_all.csv not found.** Run the ML notebook first.")
        return

    with st.spinner("Training models — cached after first run (~30s initial)…"):
        try:
            rf, xgb, ALL_F, metrics, feat_imp, y_te, xgb_pred, medians = train_models()
        except Exception as e:
            st.error(f"Model training failed: {e}"); return

    imp_color = "#10b981" if metrics["improvement_pct"] > 0 else "#ef4444"
    kpi_cols = st.columns(4)
    kpi(kpi_cols, [
        {"value": f"{metrics['baseline_rmse']:.4f}", "label": "Baseline RMSE (time only)",  "sub": "Linear regression",             "color": "#64748b"},
        {"value": f"{metrics['rf_rmse']:.4f}",       "label": "Random Forest RMSE",          "sub": "100 trees, max_depth=12",       "color": "#3b82f6"},
        {"value": f"{metrics['xgb_rmse']:.4f}",      "label": "XGBoost RMSE",                "sub": "200 trees, lr=0.05",            "color": "#10b981"},
        {"value": f"±{metrics['xgb_mae']:.1%}",      "label": "Avg Prediction Error",        "sub": f"{metrics['improvement_pct']:+.1f}% vs baseline", "color": imp_color},
    ])
    ...
    with tab1:
        info_box(
            what="Side-by-side RMSE comparison of three models: a time-only baseline (linear regression), "
                 "Random Forest (100 trees, 37 features), and XGBoost (200 trees, full feature fusion). "
                 "Right chart: each dot is one test-set prediction vs the actual occupancy.",
            why="RMSE (Root Mean Square Error) measures average prediction error in occupancy units (0–1). "
                "Lower is better. The improvement from baseline → XGBoost quantifies the value of "
                "cross-geography data fusion — the project's core research claim.",
            how="Left bar chart: shorter bar = better model. Look at the percentage drop from grey → green. "
               "Right scatter: dots close to the dashed diagonal = accurate predictions. "
               "A fan shape (wider at high values) means the model struggles at peak demand — common in ML.",
        )
        c1, c2 = st.columns(2)
        with c1:
            fig = go.Figure(go.Bar(
                x=["Baseline\n(Time only)", "Random\nForest", "XGBoost\n(Full fusion)"],
                y=[metrics["baseline_rmse"], metrics["rf_rmse"], metrics["xgb_rmse"]],
                marker_color=["#94a3b8", "#3b82f6", "#10b981"],
                text=[f"{v:.4f}" for v in [metrics["baseline_rmse"], metrics["rf_rmse"], metrics["xgb_rmse"]]],
        ...
        insight(f"XGBoost (full data fusion) achieves <strong>RMSE = {metrics['xgb_rmse']:.4f}</strong> vs baseline <strong>{metrics['baseline_rmse']:.4f}</strong> — a <strong>{metrics['improvement_pct']:.1f}% improvement</strong> from adding behavioral patterns, weather, and infrastructure features.")
    ...
    with tab3:
        info_box(
            ...
            why="Operators need to anticipate demand spikes (e.g., before a holiday weekend or during a "
                "heatwave). This tool lets anyone test scenarios without writing code.",
            how="Fill in the sliders — hour, weather, price. Click Predict. The gauge shows predicted "
               "occupancy (0–100%). Green (≤65%) = normal, Yellow (65–85%) = watch, Red (>85%) = alert. "
               "Tip: try hour=01:00 + weekend to see the overnight Shenzhen peak.",
        )
...
# ─────────────────────────────────────────────────────────────────────────────
def page_forecast():
    sh("📈 48-Hour Forecast", "ARIMA vs ARIMAX with behavioral prior — operational planning window")
    ...
    fig.add_vline(x=r["ts_te_idx"][0], line_dash="dot", line_color="grey",
                  annotation_text="48h forecast window starts")
    ...
    with c2:
        ...
        insight(f"<strong>Cross-geography behavioral transfer:</strong> Adding ACN arrival patterns as an exogenous variable <strong>{direction}</strong> forecast accuracy by <strong>{abs(r['improvement']):.1f}%</strong>. Human charging habits generalize across geographies — validating the India transfer hypothesis.")
...
def page_zone_intelligence():
    sh("🗺️ Zone Intelligence", "K-Means clustering identifies 4 zone archetypes with distinct infrastructure needs")
    ...
    tab1, tab2, tab3 = st.tabs(["📈 Archetype Profiles", "🎯 K Selection", "💡 Investment Guide"])

    with tab1:
        info_box(
            what="K-Means clustering groups the 275 Shenzhen zones into 4 archetypes based on their "
                 "hourly occupancy profile across all hours. Each line = one archetype's average demand curve.",
            why="Zones with similar demand patterns need similar infrastructure. Building the same charger type "
                "everywhere wastes capital. Archetypes let engineers prioritize zones and right-size hardware.",
            how="4 lines, each a different color — higher Y = busier zone. The gap between lines shows how "
               "different the tiers are. Cards below each chart show the recommended charger type and "
               "investment priority for that archetype.",
        )
        fig = go.Figure()
        for i in range(K_Z):
        ...
        st.plotly_chart(fig, use_container_width=True)
        insight(
            "Zones are grouped by <strong>occupancy level</strong> — how busy they are across all hours, not just peak time. "
            "Shenzhen zones all peak <strong>overnight (00:00–07:00)</strong> due to residential EV adoption. "
            "Four distinct tiers emerge: <strong>High-Demand Hubs</strong> need immediate expansion, "
            "<strong>Overnight Residential</strong> zones need overnight capacity, "
            "<strong>Moderate Steady</strong> zones are stable ROI targets, and "
            "<strong>Low Activity</strong> zones should be monitored before investing."
        )
    ...
    with tab2:
        info_box(
            what="Two statistical charts used to justify choosing k=4 clusters: the Elbow Curve "
                 "(inertia drops) and the Silhouette Score (cluster separation quality).",
            why="K (number of clusters) is a hyperparameter — there's no single right answer. "
                "These charts give objective evidence for k=4, making the model choice defensible in the paper.",
            how="Left: Inertia drops sharply until k=4, then flattens (the \"elbow\"). Right: Silhouette score "
               "peaks near the green dashed line — that's the statistically optimal k. "
               "We use k=4 for interpretability even if math suggests a slightly different value.",
        )
        ...
        with kc2:
            ...
            st.plotly_chart(fig, use_container_width=True)
        st.caption(f"Optimal k = {r['best_k']} by silhouette. We use k=4 for interpretability and alignment with land-use patterns.")

    with tab3:
        info_box(
            what="A decision matrix translating each zone archetype into a concrete infrastructure recommendation: "
                 "charger type, investment priority, suggested action, and key risk to watch.",
            why="The clustering output alone doesn't tell an operator what to build. This matrix bridges "
                "the gap from data → actionable procurement decision.",
            how="Each row = one archetype. Read: Zone Archetype (which zones) → Charger (what hardware) → "
               "Priority (how urgent) → Action (what to do now) → Risk (what to watch for). "
               "High-Demand Hubs need DC Fast chargers immediately; Low Activity zones should wait.",
        )
        ...
        for i in range(K_Z):
            ...
            rows.append({"Zone Archetype": f"{icon} {lbl}", "Zone Count": counts[i],
                         **details})
        st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)
        insight("🌏 <strong>India Transfer:</strong> Zone archetypes generalize across markets. Reassign archetype labels using local peak hours without model retraining.")


# ─────────────────────────────────────────────────────────────────────────────
def page_user_segments():
    sh("👥 User Segments", "K-Means on ACN session data reveals 4 behavioral clusters with distinct charger needs")
...
def page_station_quality():
    sh("⭐ Station Quality", "What drives user satisfaction? Correlation analysis on 5,000 global stations")
...
def page_reviews_nlp():
    sh("💬 Reviews & NLP Analysis",
       "VADER sentiment + pain point categorization on EV station user reviews")
    ...
    with tab2:
        ...
        insight("<strong>Key finding:</strong> The #1 driver of negative reviews is <strong>Equipment Reliability</strong>, not wait time or charging speed. This is an <em>operational</em> fix — not an infrastructure investment.")
...
def sidebar_nav() -> str:
    with st.sidebar:
        ...
        <div style="text-align:center;padding:16px 0 24px">
            ...
            <div style="font-weight:700;color:#e2e8f0;font-size:1.1rem">EV Intelligence</div>
            <div style="font-size:0.74rem;color:#64748b">AAI-510 · USD · 2024</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        PAGES = {
            ...
            "📈 48h Forecast":        "forecast",
            "🗺️ Zone Intelligence":   "zones",
            "👥 User Segments":       "segments",
            "⭐ Station Quality":     "quality",
            "💬 Reviews & NLP":       "nlp",
            "📋 Summary":             "summary",
        }
        ...
        for path, label in checks:
            icon = "✅" if path.exists() else ("⚠️" if "Fused" in label else "❌")
